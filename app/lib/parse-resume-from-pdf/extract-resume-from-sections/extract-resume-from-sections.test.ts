...
describe("extract-profile tests - ", () => {
  ...
  it("Name with diacritics", () => {
    expect(
      matchOnlyLetterSpaceOrPeriod(makeTextItem("Érica d'Ávila-Ferreira"))![0]
    ).toBe("Érica d'Ávila-Ferreira");
  });
  ...
  it("Phone pt-BR", () => {
    expect(matchPhone(makeTextItem("+55 (11) 99876-5432"))![0]).toBe(
      "+55 (11) 99876-5432"
    );
  });
  ...
  it("Location pt-BR", () => {
    expect(matchCityAndState(makeTextItem("São Paulo - SP"))![0]).toBe(
      "São Paulo - SP"
    );
